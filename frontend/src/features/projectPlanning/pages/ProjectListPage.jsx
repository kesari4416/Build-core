  ...
  return (
    <div className="p-8" data-testid="project-list-page">
      <div className="flex items-end justify-between mb-8 flex-wrap gap-4">
        <div>
          <div className="text-blue-600 text-[11px] uppercase tracking-[0.3em] font-semibold mb-1">Project Planning</div>
      ...
      <div className="flex flex-wrap gap-3 mb-5">
        <div className="relative flex-1 min-w-[220px] max-w-sm">
          <Search size={15} strokeWidth={2.5} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" />
          <input data-testid="project-search-input" value={search}
            ...
            className="w-full bg-white border border-slate-300 pl-9 pr-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
        </div>
        {user?.role !== "Client" && (
          <Select value={params.get("client_id") || "all"} onValueChange={(v) => setParam("client_id", v === "all" ? "" : v)}>
            <SelectTrigger data-testid="filter-client-select" className="w-44 bg-white border-slate-300 rounded-md">
        ...
        <Select value={params.get("status") || "all"} onValueChange={(v) => setParam("status", v === "all" ? "" : v)}>
          <SelectTrigger data-testid="filter-status-select" className="w-40 bg-white border-slate-300 rounded-md">
        ...
        {user?.role !== "Client" && (
          <Select value={params.get("site_engineer_id") || "all"} onValueChange={(v) => setParam("site_engineer_id", v === "all" ? "" : v)}>
            <SelectTrigger data-testid="filter-engineer-select" className="w-44 bg-white border-slate-300 rounded-md">
      ...
      {isLoading ? (
        <div className="space-y-2" data-testid="projects-loading">
          {[...Array(5)].map((_, i) => <Skeleton key={i} className="h-14 bg-white rounded-md" />)}
        </div>
      ) : isError ? (
        <div className="border border-red-200 bg-red-50 p-8 text-center text-red-600" data-testid="projects-error">
            ...
            <div className="flex items-center justify-between mt-4" data-testid="pagination">
              ...
              <div className="flex gap-2">
                <Button variant="outline" size="sm" disabled={offset === 0} data-testid="pagination-prev"
                  ...
                  className="rounded-md border-slate-300">Prev</Button>
                <Button variant="outline" size="sm" disabled={offset + PAGE_SIZE >= total} data-testid="pagination-next"
                  ...
                  className="rounded-md border-slate-300">Next</Button>
              </div>
            </div>
          )}
        </>
      ...
      <Dialog open={budgetOpen} onOpenChange={setBudgetOpen}>
        <DialogContent className="bg-white border-slate-300 rounded-md max-w-lg" data-testid="budget-breakdown-dialog">
          <DialogHeader>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {(budgetProjects || []).map((p) => (
                  <tr key={p.id} className="border-b border-slate-200" data-testid={`budget-row-${p.id}`}>
                    <td className="py-2.5 text-slate-900 font-medium">{p.name}</td>
                    <td className="py-2.5 text-slate-500">{p.client_name}</td>
                    <td className="py-2.5 text-right text-slate-600">{p.budget != null ? fmtCr(p.budget) : "—"}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr>
                  <td colSpan={2} className="py-3 text-[11px] uppercase tracking-[0.15em] text-slate-500 font-semibold">Total</td>
                  <td className="py-3 text-right font-heading font-bold text-xl text-blue-600" data-testid="budget-total">
