...
export const ProjectFinanceSummaryCard = ({ label, value, sub, icon: Icon, accent, testId }) => (
  <div className="border border-slate-200 bg-white shadow-sm p-5" data-testid={testId}>
    ...
    {sub && <div className="text-[10px] text-slate-500 mt-2 tracking-wide" data-testid={`${testId}-period`}>{sub}</div>}
  </div>
);

export const InvoiceCard = ({ inv, canWrite, onPay, projectName, clientName }) => (
  <div className="border border-slate-200 bg-white shadow-sm p-4" data-testid={`invoice-card-${inv.id}`}>
    <div className="flex flex-wrap items-center gap-3">
      <span className="font-heading font-bold text-lg text-blue-600">{inv.invoice_number}</span>
      <CommitmentStatusBadge status={inv.status} />
      <span className="text-xs text-slate-500">Due {inv.due_date || "—"}</span>
      <span className="ml-auto font-semibold text-slate-900">{fmt(inv.total)}</span>
    ...
    <div className="flex items-center gap-4 mt-2 text-xs">
      ...
      <span className="text-amber-600">Balance {fmt(inv.balance_due)}</span>
      {canWrite && inv.balance_due > 0 && inv.status !== "Cancelled" && (
        <button data-testid={`record-payment-${inv.id}`} onClick={() => onPay(inv)}
          className="ml-auto text-[10px] uppercase tracking-wide font-bold text-blue-600 hover:text-blue-700">+ Record Payment</button>
      )}
    </div>
  </div>
);
...
export default function ProjectFinancePage() {
  ...
  return (
    <div className="p-8" data-testid="project-finance-page">
      <Link to={`/admin/projects/${id}`} className="inline-flex items-center gap-1.5 text-xs uppercase tracking-[0.15em] font-semibold text-slate-500 hover:text-blue-600 mb-4">
      ...
      {s && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          <ProjectFinanceSummaryCard label="Revenue from Client" value={fmtCr(s.revenue_last_year)}
            ...
            icon={TrendingUp} accent="text-emerald-600" testId="pf-income" />
          <ProjectFinanceSummaryCard label="Expense" value={fmtCr(s.cost_last_year)}
            sub={`1 year · ${s.period_from} → ${s.period_to}`}
            icon={TrendingDown} accent="text-blue-600" testId="pf-cost" />
          <ProjectFinanceSummaryCard label="Profit" value={fmtCr(s.profit)}
            sub={`1 year · ${s.period_from} → ${s.period_to}`}
            icon={IndianRupee} accent={s.profit < 0 ? "text-red-500" : "text-emerald-600"} testId="pf-profit" />
          <ProjectFinanceSummaryCard label="Outstanding" value={fmtCr(s.outstanding_invoices)} icon={Receipt} accent="text-sky-600" testId="pf-outstanding" />
        </div>
      )}
      <div className="grid lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-3">
          <div className="text-[11px] uppercase tracking-[0.2em] text-slate-500 font-semibold">Invoices & Payments</div>
          {canFin && (
            <div className="border border-slate-200 bg-white shadow-sm p-4 flex flex-wrap items-end gap-3">
              <Input data-testid="invoice-amount-input" type="number" placeholder="Amount ₹" value={invForm.amount} onChange={(e) => setInvForm({ ...invForm, amount: e.target.value })} className="bg-white border-slate-300 rounded-md h-9 w-36" />
              <Input data-testid="invoice-due-input" type="date" value={invForm.due_date} onChange={(e) => setInvForm({ ...invForm, due_date: e.target.value })} className="bg-white border-slate-300 rounded-md h-9 w-40" />
              <Input data-testid="invoice-desc-input" placeholder="Description" value={invForm.description} onChange={(e) => setInvForm({ ...invForm, description: e.target.value })} className="bg-white border-slate-300 rounded-md h-9 flex-1 min-w-[160px]" />
          ...
          {(invoices || []).length === 0 && <div className="border border-slate-200 p-8 text-center text-xs text-slate-500" data-testid="invoices-empty">No invoices yet.</div>}
        </div>
        <div className="space-y-3">
          <div className="text-[11px] uppercase tracking-[0.2em] text-slate-500 font-semibold">Expense Log</div>
          {canExp && (
            <div className="border border-slate-200 bg-white shadow-sm p-3 flex gap-2">
              <select data-testid="expense-category-select" value={expForm.category}
                onChange={(e) => setExpForm({ ...expForm, category: e.target.value })}
                className="bg-white border border-slate-300 rounded-md h-9 text-sm text-slate-700 px-2 flex-1 min-w-0">
                <option value="">Category…</option>
                {(expCats || []).map((c) => <option key={c.id} value={c.name}>{c.name}</option>)}
              </select>
              <Input data-testid="expense-amount-input" type="number" placeholder="₹" value={expForm.amount} onChange={(e) => setExpForm({ ...expForm, amount: e.target.value })} className="bg-white border-slate-300 rounded-md h-9 w-28" />
          ...
          {(expenses || []).map((e) => (
            <div key={e.id} className="flex items-center gap-3 border border-slate-200 bg-white shadow-sm p-3 text-sm" data-testid={`expense-${e.id}`}>
              <span className="text-slate-700">{e.category}</span>
              <span className="text-xs text-slate-500">{e.expense_date}</span>
              <span className="ml-auto font-semibold text-slate-900">{fmt(e.amount)}</span>
            </div>
          ))}
          {canExp && (
            <div className="border border-slate-200 bg-white shadow-sm p-4 mt-6" data-testid="expense-categories-panel">
              <div className="text-[11px] uppercase tracking-[0.2em] text-slate-500 font-semibold mb-3">Expense Categories</div>
              <div className="flex gap-2 mb-3">
                <Input data-testid="new-category-input" placeholder="New category name" value={newCat}
                  onChange={(e) => setNewCat(e.target.value)} className="bg-white border-slate-300 rounded-md h-9" />
              ...
              <div className="space-y-1.5" data-testid="expense-categories-list">
                {(expCats || []).map((c) => (
                  <div key={c.id} className="flex items-center gap-2 border border-slate-200 bg-slate-50 px-3 py-2 text-sm" data-testid={`category-row-${c.id}`}>
                    <Tag size={12} strokeWidth={2.5} className="text-blue-600" />
                    <span className="text-slate-700">{c.name}</span>
                    <button data-testid={`edit-category-${c.id}`} title="Edit category" onClick={() => editCategory(c)}
                      className="ml-auto p-1 text-slate-500 hover:text-blue-600 transition-colors"><Pencil size={13} strokeWidth={2.5} /></button>
                    <button data-testid={`delete-category-${c.id}`} title="Delete category"
                      ...
                      className="p-1 text-slate-500 hover:text-red-600 transition-colors"><Trash2 size={13} strokeWidth={2.5} /></button>
                  </div>
                ))}
                {(expCats || []).length === 0 && <div className="text-xs text-slate-400">No categories yet.</div>}
              </div>
            </div>
          )}
        </div>
      ...
      {ledger && (
        <div className="mt-8" data-testid="ledger-section">
          <div className="flex items-center justify-between flex-wrap gap-3 mb-3">
            <div className="text-[11px] uppercase tracking-[0.2em] text-slate-500 font-semibold">Project Ledger — Credits & Debits</div>
            <div className="flex gap-4 text-xs">
              <span className="text-emerald-600" data-testid="ledger-total-credit">Credit: {fmt(ledger.total_credit)}</span>
              <span className="text-red-600" data-testid="ledger-total-debit">Debit: {fmt(ledger.total_debit)}</span>
              <span className={ledger.net >= 0 ? "text-emerald-600" : "text-red-600"} data-testid="ledger-net">Net: {fmt(ledger.net)}</span>
            </div>
          </div>
            ...
            <table className="w-full text-sm" data-testid="ledger-table">
              ...
              <tbody>
                {ledger.entries.map((en, i) => (
                  <tr key={i} className="border-b border-slate-100" data-testid={`ledger-row-${i}`}>
                    <td className="px-4 py-2.5 text-xs text-slate-500">{en.date || "—"}</td>
                    <td className="px-4 py-2.5 text-slate-700">{en.description}</td>
                    <td className="px-4 py-2.5 text-right font-semibold text-emerald-600">{en.type === "credit" ? fmt(en.amount) : ""}</td>
                    <td className="px-4 py-2.5 text-right font-semibold text-red-600">{en.type === "debit" ? fmt(en.amount) : ""}</td>
                  </tr>
                ))}
                {ledger.entries.length === 0 && (
                  <tr><td colSpan={4} className="px-4 py-8 text-center text-slate-500" data-testid="ledger-empty">No transactions yet.</td></tr>
                )}
              </tbody>
            </table>
          </div>
