...
- Change Order / Client Modification module (2026-06, APPROVED by user, TESTED iter_18: backend 16/16, frontend 100%):
  ...
  - Attachments (2026-06, user said "Dont test" — NOT verified by testing agent): create modal now has photo/PDF upload (co-attachment-button, reuses POST /api/upload, 10MB, jpg/png/webp/gif/pdf) sent as attachments[{url,filename}]; CO detail row shows clickable thumbnail gallery (co-attachments-{id}) + paperclip count on collapsed header. Backend field pre-existed.
