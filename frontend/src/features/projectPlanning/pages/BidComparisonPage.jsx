    ...
    <table className="w-full text-sm" data-testid="bid-comparison-table">
        ...
        <tr className="text-left text-[10px] uppercase tracking-[0.15em] text-slate-500 border-b border-slate-200 bg-white">
          ...
          {data.vendors.map((v) => (
            <th key={v.bid_id} className="px-4 py-3 min-w-[160px]">
              <div className="text-slate-900 normal-case tracking-normal text-sm font-semibold">{v.vendor_name}</div>
      ...
      <tbody>
        {data.line_items.map((li) => (
          <tr key={li.id} className="border-b border-slate-100" data-testid={`comparison-row-${li.id}`}>
            <td className="px-4 py-3">
              <div className="text-slate-900 font-medium">{li.item_description}</div>
              <div className="text-xs text-slate-500">{li.quantity_required} {li.unit || ""} · {li.cost_code || "—"}</div>
            </td>
            {data.vendors.map((v) => {
              const q = li.quotes.find((x) => x.bid_id === v.bid_id);
              return (
                <td key={v.bid_id} className={`px-4 py-3 ${q?.is_best ? "bg-emerald-500/5" : ""}`} data-testid={`quote-cell-${li.id}-${v.vendor_id}`}>
                  {q ? (
                    <div>
                      <div className="flex items-center gap-2">
                        <span className={`font-semibold ${q.is_best ? "text-emerald-600" : "text-slate-900"}`}>{fmt(q.unit_price)}</span>
                  ...
                  ) : <span className="text-slate-400 text-xs">No quote</span>}
                </td>
              );
            })}
          </tr>
        ))}
        <tr className="bg-white" data-testid="comparison-totals-row">
          <td className="px-4 py-3 text-[10px] uppercase tracking-[0.15em] text-slate-500 font-semibold">Bid Total</td>
          {data.vendors.map((v) => {
            const best = Math.min(...data.vendors.filter((x) => x.total > 0).map((x) => x.total));
            return (
              <td key={v.bid_id} className="px-4 py-3">
                <div className={`font-heading font-bold text-xl ${v.total === best && v.total > 0 ? "text-emerald-600" : "text-slate-900"}`}>{fmt(v.total)}</div>
                {isAdmin && data.status !== "Awarded" && v.status !== "Awarded" && (
                  <div className="flex gap-1.5 mt-2">
                    <button data-testid={`award-po-${v.vendor_id}`} onClick={() => onAward(v, "po")}
                      className="border border-blue-600/50 text-blue-600 hover:bg-blue-600 hover:text-white px-2 py-1 text-[10px] uppercase tracking-wide font-bold transition-colors">
                      Award PO
                    </button>
                    <button data-testid={`award-sub-${v.vendor_id}`} onClick={() => onAward(v, "subcontract")}
                      className="border border-slate-300 text-slate-500 hover:border-blue-400 hover:text-blue-600 px-2 py-1 text-[10px] uppercase tracking-wide font-bold transition-colors">
                      Award Sub
                    </button>
                  </div>
                )}
                {v.status === "Awarded" && (
                  <span className="flex items-center gap-1 mt-2 text-[10px] uppercase tracking-wide font-bold text-emerald-600">
...
export default function BidComparisonPage() {
  ...
  return (
    <div className="p-8" data-testid="bid-comparison-page">
      <Link to="/admin/procurement/vendors" className="inline-flex items-center gap-1.5 text-xs uppercase tracking-[0.15em] font-semibold text-slate-500 hover:text-blue-600 mb-4">
        <ArrowLeft size={14} strokeWidth={2.5} /> Vendors & Quotes
      </Link>
      <div className="flex items-end justify-between flex-wrap gap-4 mb-6">
        <div>
          <div className="text-blue-600 text-[11px] uppercase tracking-[0.3em] font-semibold mb-1">Quote Comparison</div>
          ...
          <div className="flex items-center gap-3 mt-3">
            ...
            <span className="text-xs text-slate-500">Bids due {data?.bid_due_date || "—"}</span>
          </div>
        </div>
      </div>
      {canEdit && data?.status !== "Awarded" && (
        <div className="border border-slate-200 bg-white shadow-sm p-4 flex flex-wrap items-end gap-3 mb-6">
          <Input data-testid="li-desc-input" placeholder="Line item description" value={liForm.item_description}
            onChange={(e) => setLiForm({ ...liForm, item_description: e.target.value })} className="bg-white border-slate-300 rounded-md h-9 flex-1 min-w-[200px]" />
          <Input data-testid="li-unit-input" placeholder="Unit" value={liForm.unit}
            onChange={(e) => setLiForm({ ...liForm, unit: e.target.value })} className="bg-white border-slate-300 rounded-md h-9 w-24" />
          <Input data-testid="li-qty-input" type="number" placeholder="Qty" value={liForm.quantity_required}
            onChange={(e) => setLiForm({ ...liForm, quantity_required: e.target.value })} className="bg-white border-slate-300 rounded-md h-9 w-24" />
      ...
      {isLoading && <div className="border border-slate-200 p-10 text-center text-slate-500">Loading comparison…</div>}
      {data && data.vendors.length === 0 && (
        <div className="border border-slate-200 p-10 text-center text-slate-500" data-testid="comparison-empty">No bids submitted yet for this package.</div>
      )}
