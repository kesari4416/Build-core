...
const Card = ({ label, value, sub, icon: Icon, accent, testId }) => (
  <div className="border border-slate-200 bg-white shadow-sm p-5" data-testid={testId}>
    ...
    {sub && <div className="text-[10px] text-slate-500 mt-2 tracking-wide">{sub}</div>}
  </div>
);
  ...
  return (
    <div data-testid="project-balance-sheet">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <Card label="Total Budget" value={fmtCr(bs.budget)} icon={IndianRupee} accent="text-sky-600" testId="pbs-budget" />
        <Card label="Client Payment (In)" value={fmtCr(bs.client_paid)} sub={`Outstanding from client: ${fmt(bs.client_outstanding)}`} icon={TrendingUp} accent="text-emerald-600" testId="pbs-client-paid" />
        <Card label="Payment Released (Out)" value={fmtCr(bs.total_released)} icon={TrendingDown} accent="text-red-600" testId="pbs-released" />
        <Card label="Balance (In − Out)" value={fmtCr(bs.balance)} sub={`Budget remaining: ${fmtCr(bs.budget_remaining)}`} icon={Wallet} accent={bs.balance < 0 ? "text-red-600" : "text-emerald-600"} testId="pbs-balance" />
      </div>

      <div className="border border-slate-200 bg-white shadow-sm p-4 mb-6" data-testid="pbs-released-breakdown">
        <div className="text-[11px] uppercase tracking-[0.2em] text-slate-500 font-semibold mb-3">Where the money is going — Payment Released Breakdown</div>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 text-sm">
          {[
            ...
            <div key={tid} className="border border-slate-200 bg-slate-50 p-3" data-testid={tid}>
              <div className="text-[10px] uppercase tracking-[0.15em] text-slate-500 font-semibold">{label}</div>
              <div className="font-semibold text-slate-900 mt-1.5">{fmt(amt)}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="flex items-center justify-between flex-wrap gap-3 mb-3">
        <div className="text-[11px] uppercase tracking-[0.2em] text-slate-500 font-semibold">All Transactions — Latest First</div>
        <div className="flex gap-4 text-xs">
          <span className="text-emerald-600" data-testid="pbs-tx-credit">Credit: {fmt(bs.total_credit)}</span>
          <span className="text-red-600" data-testid="pbs-tx-debit">Debit: {fmt(bs.total_debit)}</span>
        </div>
      </div>
      <div className="border border-slate-200 overflow-x-auto">
        <table className="w-full text-sm" data-testid="pbs-transactions-table">
          ...
          <tbody>
            {bs.entries.map((en, i) => (
              <tr key={i} className="border-b border-slate-100" data-testid={`pbs-tx-row-${i}`}>
            ...
            {bs.entries.length === 0 && (
              <tr><td colSpan={4} className="px-4 py-8 text-center text-slate-500" data-testid="pbs-tx-empty">No transactions yet.</td></tr>
            )}
          </tbody>
        </table>
      </div>
