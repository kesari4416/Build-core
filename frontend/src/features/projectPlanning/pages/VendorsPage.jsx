...
export default function VendorsPage() {
  ...
  return (
    <div className="p-8" data-testid="vendors-page">
      ...
      <div className="text-[11px] uppercase tracking-[0.2em] text-slate-500 font-semibold mb-3">Vendor Directory</div>
      <div className="border border-slate-200 overflow-x-auto mb-10">
        <table className="w-full text-sm" data-testid="vendors-table">
          ...
          <tbody>
            {(vendors || []).map((v) => (
              <tr key={v.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors" data-testid={`vendor-row-${v.id}`}>
                <td className="px-4 py-3">
                  <div className="flex items-center gap-3">
                    <div className="bg-slate-200 border border-slate-300 p-2">
                      <Truck size={15} strokeWidth={2.5} className="text-blue-600" />
                    </div>
                    <div>
                      <div className="font-semibold text-slate-900">{v.name}</div>
                      <div className="text-xs text-slate-500">{v.vendor_type}{v.prequalified ? " · Prequalified" : ""}</div>
                    </div>
                  </div>
                </td>
                <td className="px-4 py-3 text-slate-600">{v.trade || "—"}</td>
                <td className="px-4 py-3 text-xs text-slate-500">
                  <div>{v.contact_name || "—"}</div>
                  <div>{v.email || "—"}</div>
                </td>
                <td className="px-4 py-3">
                  <span className={`flex items-center gap-1.5 text-xs ${v.insurance_expiring ? "text-red-600" : "text-emerald-600"}`}>
                ...
                <td className="px-4 py-3 text-center">
                  <span className="inline-flex items-center gap-1 text-slate-600">
                    <Star size={12} strokeWidth={2.5} className="text-amber-600" /> {v.rating ?? "—"}
                  </span>
                </td>
            ...
            {(vendors || []).length === 0 && (
              <tr><td colSpan={6} className="px-4 py-10 text-center text-slate-500">No vendors yet.</td></tr>
            )}
          </tbody>
        </table>
      </div>

      <div className="text-[11px] uppercase tracking-[0.2em] text-slate-500 font-semibold mb-3">Bid Packages — Quote Comparison</div>
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4" data-testid="bid-packages-grid">
        ...
        {(packages || []).length === 0 && (
          <div className="border border-slate-200 p-10 text-center text-slate-500 md:col-span-3">No bid packages yet. Create them from a project's Procurement dashboard.</div>
        )}
      </div>
