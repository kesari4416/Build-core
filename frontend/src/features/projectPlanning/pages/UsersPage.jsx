  ...
  return (
    <div className="p-8" data-testid="users-page">
      <div className="flex items-end justify-between mb-8 flex-wrap gap-4">
        <div>
          <div className="text-blue-600 text-[11px] uppercase tracking-[0.3em] font-semibold mb-1">Administration</div>
      ...
      <div className="flex flex-wrap gap-3 mb-6">
        <Select value={role} onValueChange={setRole}>
          <SelectTrigger data-testid="users-role-filter" className="w-48 bg-white border-slate-300 rounded-md"><SelectValue /></SelectTrigger>
        ...
        <Select value={status} onValueChange={setStatus}>
          <SelectTrigger data-testid="users-status-filter" className="w-40 bg-white border-slate-300 rounded-md"><SelectValue /></SelectTrigger>
          <SelectContent className="bg-white border-slate-300">
          ...
          <tbody>
            {(users || []).map((u) => (
              <tr key={u.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors" data-testid={`user-row-${u.id}`}>
                ...
                <td className="px-4 py-3 text-right text-slate-600">{fmt(u.base_salary)}</td>
                <td className="px-4 py-3 text-xs text-slate-500">{u.last_login_at ? u.last_login_at.slice(0, 10) : "Never"}</td>
                <td className="px-4 py-3">
                  <div className="flex items-center justify-end gap-1">
                    {(u.role === "SiteEngineer" || u.role === "ProcurementOfficer") && (
                      <button data-testid={`assign-user-${u.id}`} title="Assign to project" onClick={() => setAssignUser(u)}
                        className="p-1.5 text-slate-500 hover:text-blue-600 transition-colors"><HardHat size={15} strokeWidth={2.5} /></button>
                    )}
                    ...
                    <button data-testid={`reset-password-${u.id}`} title="Reset password" onClick={() => resetPassword(u)}
                      className="p-1.5 text-slate-500 hover:text-amber-600 transition-colors"><KeyRound size={15} strokeWidth={2.5} /></button>
                    {u.status !== "Disabled" ? (
                      ...
                      <button data-testid={`enable-user-${u.id}`} title="Enable" onClick={() => run(() => api.patch(`/users/${u.id}`, { status: "Active" }), `${u.name} enabled`)}
                        className="p-1.5 text-slate-500 hover:text-emerald-600 transition-colors"><UserCheck size={15} strokeWidth={2.5} /></button>
                    )}
                    <button data-testid={`delete-user-${u.id}`} title="Delete"
                      ...
                      className="p-1.5 text-slate-500 hover:text-red-500 transition-colors"><Trash2 size={15} strokeWidth={2.5} /></button>
                  </div>
                </td>
              </tr>
            ))}
            {(users || []).length === 0 && (
              <tr><td colSpan={6} className="px-4 py-10 text-center text-slate-500" data-testid="users-empty">No users match the filters.</td></tr>
            )}
          </tbody>
        </table>
      </div>
