...
const CELL = {
  ...
  half_day: "bg-amber-500/15 text-amber-600 border-amber-500/30",
  absent: "bg-red-500/15 text-red-600 border-red-200",
  leave: "bg-sky-500/15 text-sky-600 border-sky-500/30",
};
...
export const AttendanceMarkGrid = ({ employees, dates, attendanceMap, onMark }) => (
  <div className="border border-slate-200 overflow-x-auto">
    <table className="w-full text-sm" data-testid="attendance-grid">
        ...
        <tr className="text-left text-[10px] uppercase tracking-[0.15em] text-slate-500 border-b border-slate-200 bg-white">
          ...
          {dates.map((d) => (
            <th key={d} className="px-2 py-3 text-center min-w-[64px]">
              ...
              <div className="text-slate-500 normal-case">{d.slice(8)}/{d.slice(5, 7)}</div>
            </th>
          ))}
        </tr>
      </thead>
      <tbody>
        {employees.map((e) => (
          <tr key={e.id} className="border-b border-slate-100" data-testid={`grid-row-${e.id}`}>
            <td className="px-4 py-2">
              <div className="font-semibold text-slate-900">{e.name}</div>
              <div className="text-[11px] text-slate-500">{e.role_title || "—"}</div>
            </td>
            {dates.map((d) => {
                ...
                <td key={d} className="px-2 py-2 text-center">
                  <button data-testid={`grid-cell-${e.id}-${d}`}
                    ...
                    className={`w-9 h-9 border font-heading font-bold text-sm transition-colors ${
                      status ? CELL[status] : "border-slate-200 text-slate-700 hover:border-blue-400 hover:text-blue-600"}`}>
        ...
        {employees.length === 0 && (
          <tr><td colSpan={dates.length + 1} className="px-4 py-8 text-center text-slate-500">No active employees.</td></tr>
        )}
      </tbody>
    </table>
  </div>
