    ...
    <div className="space-y-8" data-testid="employees-tab-content">
      <div>
        <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
          <div className="text-[11px] uppercase tracking-[0.2em] text-slate-500 font-semibold flex items-center gap-2">
        ...
        <div className="grid md:grid-cols-2 gap-3" data-testid="daily-attendance-list">
          ...
          {active.length === 0 && (
            <div className="border border-slate-200 p-8 text-center text-slate-500 md:col-span-2" data-testid="employees-empty">
      ...
      {active.length > 0 && (
        <div>
          <div className="text-[11px] uppercase tracking-[0.2em] text-slate-500 font-semibold mb-3">Attendance — Last 7 Days (click a cell to mark/cycle)</div>
          <AttendanceMarkGrid employees={active} dates={dates} attendanceMap={attMap} onMark={mark} />
        </div>
      )}

      <div>
        <div className="text-[11px] uppercase tracking-[0.2em] text-slate-500 font-semibold mb-3">Employee Register · {(employees || []).length}</div>
        <div className="border border-slate-200 overflow-x-auto">
          <table className="w-full text-sm" data-testid="employees-table">
            ...
            <tbody>
              {(employees || []).map((e) => (
                <tr key={e.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors" data-testid={`employee-row-${e.id}`}>
                  <td className="px-4 py-2.5 font-semibold text-slate-900">{e.name}</td>
                  <td className="px-4 py-2.5 text-slate-600">{e.role_title || "—"}</td>
                  <td className="px-4 py-2.5 text-xs text-slate-500">{e.phone || "—"}</td>
                  <td className="px-4 py-2.5 text-right text-slate-700">{e.daily_wage != null ? `${fmt(e.daily_wage)}/${e.wage_type === "daily" ? "day" : e.wage_type}` : "—"}</td>
                  <td className="px-4 py-2.5 text-xs text-slate-500">{e.joining_date || "—"}</td>
                  ...
                  <td className="px-4 py-2.5">
                    <div className="flex items-center justify-end gap-1">
                      <button data-testid={`edit-employee-${e.id}`} title="Edit" onClick={() => setModal({ open: true, employee: e })}
                        className="p-1.5 text-slate-500 hover:text-blue-600 transition-colors"><Pencil size={15} strokeWidth={2.5} /></button>
                      {e.status === "active" && canManageWage && (
                        <button data-testid={`deactivate-employee-${e.id}`} title="Deactivate" onClick={() => deactivate(e)}
                          className="p-1.5 text-slate-500 hover:text-red-600 transition-colors"><UserX size={15} strokeWidth={2.5} /></button>
                      )}
                    </div>
                  </td>
                </tr>
              ))}
              {(employees || []).length === 0 && <tr><td colSpan={7} className="px-4 py-8 text-center text-slate-500">No employees yet.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-3 flex-wrap gap-3">
          ...
          <div className="flex items-center gap-2">
            <Input data-testid="labour-from-input" type="date" value={range.from} onChange={(e) => setRange({ ...range, from: e.target.value })} className="bg-white border-slate-300 rounded-md h-8 w-36 text-xs" />
            <span className="text-slate-400 text-xs">→</span>
            <Input data-testid="labour-to-input" type="date" value={range.to} onChange={(e) => setRange({ ...range, to: e.target.value })} className="bg-white border-slate-300 rounded-md h-8 w-36 text-xs" />
          </div>
        </div>
        <div className="border border-slate-200 overflow-x-auto">
          <table className="w-full text-sm" data-testid="labour-cost-table">
            ...
            <tbody>
              {(labour?.rows || []).map((r) => (
                <tr key={r.employee_id} className="border-b border-slate-100" data-testid={`labour-row-${r.employee_id}`}>
                  <td className="px-4 py-2.5">
                    <span className="font-semibold text-slate-900">{r.name}</span>
                    <span className="text-xs text-slate-500 ml-2">{r.role_title || ""}</span>
                  </td>
                  <td className="px-4 py-2.5 text-center font-heading font-bold text-lg text-slate-700">{r.days_present}</td>
                  <td className="px-4 py-2.5 text-right text-slate-500">{r.daily_wage != null ? fmt(r.daily_wage) : "—"}</td>
                  <td className="px-4 py-2.5 text-right font-semibold text-slate-900">{r.amount != null ? fmt(r.amount) : <span className="text-slate-400 text-xs">manual ({r.wage_type})</span>}</td>
                </tr>
              ))}
              <tr className="bg-white">
                <td colSpan={3} className="px-4 py-3 text-[10px] uppercase tracking-[0.15em] text-slate-500 font-semibold">Total Labour Cost · {labour?.date_from} → {labour?.date_to}</td>
                <td className="px-4 py-3 text-right font-heading font-bold text-xl text-blue-600" data-testid="labour-total">{fmt(labour?.total_amount)}</td>
              </tr>
            </tbody>
          </table>
        </div>
