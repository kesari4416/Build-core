...
const Cell = ({ label, value, testId }) => (
  <div className="border border-zinc-800 bg-zinc-900/60 p-4" data-testid={testId}>
    <div className="text-[10px] uppercase tracking-[0.2em] text-zinc-500 font-semibold">{label}</div>
    <div className="font-heading font-semibold text-lg mt-1.5 leading-none text-white">{value ?? "—"}</div>
  </div>
);
...
export default function CommitmentDetailPage() {
  ...
  if (isLoading || !c)
    return <div className="p-8"><Skeleton className="h-16 bg-zinc-900 rounded-none w-1/2" /></div>;
  ...
  return (
    <div className="p-8" data-testid="commitment-detail-page">
      <Link to={`/admin/projects/${id}/procurement`} data-testid="back-to-procurement" className="inline-flex items-center gap-1.5 text-xs uppercase tracking-[0.15em] font-semibold text-zinc-500 hover:text-orange-500 transition-colors mb-4">
      ...
      <div className="flex flex-wrap items-center gap-3 mb-2">
        ...
        {c.pending_approval && <span className="text-[11px] uppercase tracking-wide text-yellow-400 border border-yellow-500/40 bg-yellow-500/10 px-2 py-0.5 font-semibold">Pending Items</span>}
      </div>
      <div className="text-sm text-zinc-500 mb-6">{c.vendor_name} · {type === "po" ? "Purchase Order" : "Subcontract"} · Cost code {c.cost_code || "—"}</div>
      ...
      <Tabs defaultValue="overview">
        <TabsList className="bg-transparent border-b border-zinc-800 rounded-none w-full justify-start h-auto p-0 gap-1">
          {["overview", "change-orders", "pay-apps", "documents"].map((t) => (
            <TabsTrigger key={t} value={t} data-testid={`ctab-${t}`}
              className="rounded-none px-4 py-2.5 text-xs uppercase tracking-[0.15em] font-semibold data-[state=active]:bg-zinc-900 data-[state=active]:text-white data-[state=active]:border-b-2 data-[state=active]:border-orange-500 data-[state=active]:shadow-none text-zinc-500 border-b-2 border-transparent">
              {t.replace("-", " ")}
            </TabsTrigger>
          ))}
        </TabsList>
            ...
            <div className="flex gap-3 flex-wrap">
              {type === "po" && c.status !== "Approved" && c.status !== "Cancelled" && (
                <Button data-testid="approve-commitment" onClick={() => act({ url: `${base}/approve` }, "PO approved")}
                  className="rounded-none bg-green-600 hover:bg-green-700 text-white font-bold uppercase tracking-wide">Approve PO</Button>
              )}
              {type === "subcontract" && c.status !== "Executed" && (
                <>
                  <Button data-testid="approve-commitment" onClick={() => act({ url: `${base}/approve` }, "Subcontract approved")}
                    className="rounded-none bg-green-600 hover:bg-green-700 text-white font-bold uppercase tracking-wide">Approve</Button>
                  <Button data-testid="execute-commitment" onClick={() => act({ url: `${base}/execute` }, "Subcontract executed")}
                    className="rounded-none bg-orange-500 hover:bg-orange-600 text-zinc-950 font-bold uppercase tracking-wide">Execute</Button>
                </>
              )}
              {type === "po" && c.status !== "Cancelled" && (
                <Button variant="outline" data-testid="cancel-commitment" onClick={() => act({ url: `${base}/cancel` }, "PO cancelled")}
                  className="rounded-none border-red-500/50 text-red-400 hover:bg-red-500/10 uppercase tracking-wide">Cancel</Button>
              )}
            </div>
          )}
          {type === "po" && (
            <div>
              <div className="text-[11px] uppercase tracking-[0.2em] text-zinc-500 font-semibold mb-3">PO Line Items</div>
        ...
        <TabsContent value="change-orders" className="mt-6 space-y-4" data-testid="ctab-cos-content">
          {canWrite && (
            <div className="border border-zinc-800 bg-zinc-900/60 p-4 flex flex-wrap items-end gap-3">
              <div className="flex-1 min-w-[220px]">
                <div className="text-[10px] uppercase tracking-[0.15em] text-zinc-500 mb-1">Reason</div>
              ...
              <div>
                <div className="text-[10px] uppercase tracking-[0.15em] text-zinc-500 mb-1">Amount (₹, +/−)</div>
                <Input data-testid="co-amount-input" type="number" value={coForm.amount} onChange={(e) => setCoForm({ ...coForm, amount: e.target.value })}
                  className="bg-zinc-950 border-zinc-700 rounded-none h-9 w-40" />
              </div>
          ...
          {(!cos || cos.length === 0) && <div className="border border-zinc-800 p-8 text-center text-xs text-zinc-500" data-testid="cos-empty">No change orders.</div>}
        </TabsContent>
        ...
        <TabsContent value="pay-apps" className="mt-6 space-y-4" data-testid="ctab-payapps-content">
          {canWrite && (
            <div className="border border-zinc-800 bg-zinc-900/60 p-4 flex flex-wrap items-end gap-3">
              <div>
                <div className="text-[10px] uppercase tracking-[0.15em] text-zinc-500 mb-1">Amount this period (₹)</div>
                <Input data-testid="payapp-amount-input" type="number" value={paForm.amount_this_period}
                  ...
                  className="bg-zinc-950 border-zinc-700 rounded-none h-9 w-48" />
              </div>
              ...
              {type === "subcontract" && <span className="text-[11px] text-zinc-500">Retainage {c.retainage_pct}% is withheld automatically</span>}
            </div>
          )}
          {(payApps || []).map((pa) => (
            <div key={pa.id} className="border border-zinc-800 bg-zinc-900/60 p-5 space-y-4" data-testid={`payapp-card-${pa.id}`}>
              <div className="flex flex-wrap items-center gap-3">
                <span className="font-heading font-bold text-xl text-orange-500">App #{pa.application_number}</span>
                ...
                <div className="ml-auto flex gap-2">
                  {canWrite && pa.status === "Draft" && (
                    <Button size="sm" data-testid={`payapp-submit-${pa.id}`} onClick={() => act({ url: `/pay-applications/${pa.id}/submit` }, "Pay app submitted")}
                      className="rounded-none bg-sky-600 hover:bg-sky-700 text-white text-xs uppercase h-8">Submit</Button>
                  )}
                  {isAdmin && ["Submitted", "UnderReview"].includes(pa.status) && (
                    <Button size="sm" data-testid={`payapp-approve-${pa.id}`} onClick={() => act({ url: `/pay-applications/${pa.id}/approve` }, "Pay app approved")}
                      className="rounded-none bg-green-600 hover:bg-green-700 text-white text-xs uppercase h-8">Approve</Button>
                  )}
                  {isAdmin && pa.status === "Approved" && (
                    <Button size="sm" data-testid={`payapp-paid-${pa.id}`} onClick={() => act({ url: `/pay-applications/${pa.id}/mark-paid` }, "Marked paid")}
                      className="rounded-none bg-green-700 hover:bg-green-800 text-white text-xs uppercase h-8">Mark Paid</Button>
                  )}
                </div>
              </div>
              <div className="grid grid-cols-3 gap-3 text-sm">
                <div><span className="text-[10px] uppercase tracking-wide text-zinc-500 block">This Period</span><span className="font-semibold text-white">{fmt(pa.amount_this_period)}</span></div>
                <div><span className="text-[10px] uppercase tracking-wide text-zinc-500 block">Retainage Held</span><span className="font-semibold text-yellow-400" data-testid={`payapp-retainage-${pa.id}`}>{fmt(pa.retainage_held)}</span></div>
                <div><span className="text-[10px] uppercase tracking-wide text-zinc-500 block">Amount Due</span><span className="font-semibold text-green-400">{fmt(pa.amount_due)}</span></div>
              </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] uppercase tracking-[0.2em] text-zinc-500 font-semibold">Lien Waivers</span>
                  {canWrite && (
                    <button data-testid={`waiver-add-${pa.id}`}
                      ...
                      className="text-[10px] uppercase tracking-wide font-bold text-orange-500 hover:text-orange-400">+ Add Waiver</button>
                  )}
                </div>
          ...
          {(!payApps || payApps.length === 0) && <div className="border border-zinc-800 p-8 text-center text-xs text-zinc-500" data-testid="payapps-empty">No pay applications.</div>}
        </TabsContent>
