...
- P2: Replace window.prompt/confirm dialogs (record payment, reset password, delete user, award, CO approve/reject) with shadcn Dialogs; email channel for alerts (needs Resend/SendGrid key); Gantt view; export reports; invoice PDF export; file attachments on change orders (backend field exists, no upload UI)
